...
namespace Unity.XR.XREAL
{
    ...
    public class XREALAudioCapture : IDisposable
    {
        ...
        {
            // Debug.LogWarning("[AudioCapture] OnAudioDataCallback: size={0}, data={1}", size, data);
            if (m_AudioFilterStream != null)
            {
            ...
            if (recordApp)
            {
                XREALAndroidPermissionsManager.GetInstance().RequestAndroidPermission("android.permission.RECORD_AUDIO").ThenAction((requestResult) =>
                {
                    if (requestResult.IsAllGranted)
                    {
                        XREALAndroidPermissionsManager.GetInstance().RequestScreenCapture().ThenAction((AndroidJavaObject mediaProjection) =>
                        {
                            ...
                            else
                            {
                                ...
                                //NRSessionManagerExtension.HandleKernalError(new NRPermissionDenyError(NativeConstants.ScreenCaptureDenyErrorTip));
                            }
                        });
                    }
                    else
                    {
                        ...
                        //NRSessionManagerExtension.HandleKernalError(new NRPermissionDenyError(NativeConstants.AudioPermissionDenyErrorTip));
                    }
                });
            }
            else if (recordMic)
